...
Looking back, I probably could've structured the code a bit better to make actual unit tests easier (or do Test-Driven Development), but my focus was on getting something that does the intended job without overcomplicating the code. (I know from past projects that the seemingly trivial task of downloading something/working with network requests can be quite tricky.)